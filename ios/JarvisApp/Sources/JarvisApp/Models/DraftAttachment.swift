...
struct DraftAttachment: Identifiable, Equatable {
    ...
    static func image(_ img: UIImage, name: String) -> DraftAttachment? {
        ...
        return DraftAttachment(kind: .image, name: name, mimeType: "image/jpeg", data: data, image: img)
    }
    ...
    static func file(data: Data, name: String, mimeType: String) -> DraftAttachment {
        ...
        return DraftAttachment(kind: img != nil ? .image : .file, name: name, mimeType: mimeType, data: data, image: img)
    }

    /// Wire shape the server's `extractAttachmentFiles` expects: { name, mimeType, data(base64), size }.
    var payload: [String: Any] {
        ["name": name, "mimeType": mimeType, "data": data.base64EncodedString(), "size": size]
    }
}
