...
```
groups/payne/
├── CLAUDE.md                  persona + rules + reference to constraints.md + muscle_groups.md
├── constraints.md             injuries, equipment, frequency, goals
├── muscle_groups.md           canonical vocabulary of muscle-group slugs (see §3.4)
├── profile.md                 filled after intake
├── exercises/                 image cache + per-exercise metadata
│   ├── incline-db-press.jpg
│   ├── incline-db-press.json  { name_ru, name_en, primary_muscle_groups, secondary_muscle_groups, equipment, axial_load, refs, notes }
│   └── ...
├── programs/
...
```json
{
  ...
  "primary_muscle_groups": ["chest_upper"],
  "secondary_muscle_groups": ["triceps", "delts_front"],
...
`primary_muscle_groups` are the main targets (usually 1, sometimes 2). `secondary_muscle_groups` are assistants that get meaningful load (usually 1–3). Both fields use slugs from the canonical vocabulary in §3.4. The split is used by the swap-validation rule in §5.4.
...
### 3.4 Muscle-groups vocabulary — `muscle_groups.md`

Fixed slug list, committed to the repo. Payne reads this file in its system prompt and is required to use only these slugs in `exercises/*.json` cards. New slugs may be added only by editing this file.
...
core_abs          — пресс
core_obliques     — косые
lumbar_erectors   — поясничный разгибатель (нагрузка под запретом из constraints.md)
```

### 3.5 `constraints.md` (initial state)
...
**Match rule:** a proposed exercise is acceptable as a replacement iff its `primary_muscle_groups` shares at least one slug with the original's `primary_muscle_groups`, **and** the proposed exercise's `axial_load`/constraints don't violate `constraints.md`. The shared-primary rule is stricter and more interpretable than a percentage threshold — "у тебя ничего общего по основным мышцам" reads cleanly when rejecting.

- **Without `proposed`:** Payne picks 2–3 exercises with `primary_muscle_groups` intersecting the original's, filtered by `constraints.md`, sorted by (1) equipment proximity, (2) overlap size in secondary groups. Returns `exercise_swap_options` with `alternatives`.
- **With `proposed`:** Payne resolves the free-text proposal to a slug (matching existing `exercises/*.json` or creating a stub card with `image: null` and `notes: "нужна картинка"`). Applies the match rule. If accepted → `accepted: {slug}` plus the resolved card; if not → `rejected: {slug, reason}` (human reason — e.g. «целишь дельты, а исходное было на грудь») + `alternatives`.
