...
## 8. SpO2 verification gate (риск vo2max=0)

SpO2 может оказаться пустым (регион-ограничение модели Apple Watch, не включён Blood Oxygen, не пишется). Чтобы не повторить тихий `vo2max=0`:

1. **Ручная проверка (Сергей, до/во время сборки):** Health app → Дыхание → Кислород в крови — есть ли сэмплы за последние дни.
2. **Build-time диагностика (iOS):** при первом сборе логировать число `oxygenSaturation`-сэмплов за 14д (вывести в UI debug/лог), чтобы подтвердить поток.
3. **Решение:** детекцию по `spo2Min` и вклад в recovery включаем, только если данные реально текут. Если 0 — поля остаются опциональными (graceful), но CLAUDE.md SpO2 как сигнал не активирует (как vo2max). Скрипт от пустого SpO2 не ломается в любом случае.
...
- **SpO2-доступность** — главный риск (см. §8); закрывается verification gate до включения детекции.
